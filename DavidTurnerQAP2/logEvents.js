...
const logEvents = async (event, level, msg, theStatusCode) => {
  const dateTime = `${format(new Date(), "MMM-dd-yyyy \tHH:mm:ss")}`;
  const fileLineItem = `${dateTime} \t${level} \t${theStatusCode} \t${event} \t${msg} \t${uuid()}`;
  ...
  try {
    if (level === "successful") {
      ...
      const fileName =
        `${format(new Date(), "yyyyMMdd")}` + "_Successful_Events.log";
      ...
      const fileName =
        `${format(new Date(), "yyyyMMdd")}` + "_Error_Events.log";
      await fsPromises.appendFile(
        path.join(__dirname, "logs", fileName),
        fileLineItem + "\n"
      );
    } else if (level === "redirect") {
      if (!fs.existsSync(path.join(__dirname, "logs"))) {
        await fsPromises.mkdir(path.join(__dirname, "logs"));
      }
      const fileName =
        `${format(new Date(), "yyyyMMdd")}` + "_Redirected_Events.log";
      await fsPromises.appendFile(
        path.join(__dirname, "logs", fileName),
        fileLineItem + "\n"
      );
